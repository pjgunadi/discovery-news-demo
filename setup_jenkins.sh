...
jenkins/gitlab.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Create AWS Access Key
sed -e "s#\(<secret>\).*\(</secret>\)#\1$AWS_ACCESS_KEY\2#" \
jenkins/aws_access_key.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Create AWS Secret Key
sed -e "s#\(<secret>\).*\(</secret>\)#\1$AWS_SECRET_KEY\2#" \
jenkins/aws_secret_key.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Create Bluemix Login
sed -e "s#\(<username>\).*\(</username>\)#\1$BLUEMIX_USER\2#" \
-e "s#\(<password>\).*\(</password>\)#\1$BLUEMIX_PASSWORD\2#" \
jenkins/bluemix_login.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Create CloudFoundry Login
sed -e "s#\(<username>\).*\(</username>\)#\1$CF_USER\2#" \
-e "s#\(<password>\).*\(</password>\)#\1$CF_PASSWORD\2#" \
jenkins/cf_login.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Discovery News Username
sed -e "s#\(<secret>\).*\(</secret>\)#\1$DISCOVERY_USERNAME\2#" \
jenkins/discovery_username.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Discovery News Password
sed -e "s#\(<secret>\).*\(</secret>\)#\1$DISCOVERY_PASSWORD\2#" \
jenkins/discovery_password.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Docker Registry
sed -e "s#\(<username>\).*\(</username>\)#\1$DOCKER_REGISTRY_USER\2#" \
-e "s#\(<password>\).*\(</password>\)#\1$DOCKER_REGISTRY_PASSWORD\2#" \
jenkins/docker_registry.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#IBM Cloud Infrastructure Username
sed -e "s#\(<secret>\).*\(</secret>\)#\1$IBM_SL_USERNAME\2#" \
jenkins/ibm_sl_username.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#IBM Cloud Infrastructure API Key
sed -e "s#\(<secret>\).*\(</secret>\)#\1$IBM_SL_API_KEY\2#" \
jenkins/ibm_sl_api_key.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
#Create Private Key
...
jenkins/private_key.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"
...
jenkins/public_key.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
create-credentials-by-xml system::system::jenkins "(global)"

#Get Plugins Versions
WORKFLOW_JOB_PLUGIN="workflow-job"
WORKFLOW_JOB_PLUGIN_VERSION=$(java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN list-plugins | awk -v key="$WORKFLOW_JOB_PLUGIN" '$1==key {print $NF}')
GITLAB_PLUGIN="gitlab-plugin"
GITLAB_PLUGIN_VERSION=$(java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN list-plugins | awk -v key="$GITLAB_PLUGIN" '$1==key {print $NF}')
WORKFLOW_CPS_PLUGIN="workflow-cps"
WORKFLOW_CPS_PLUGIN_VERSION=$(java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN list-plugins | awk -v key="$WORKFLOW_CPS_PLUGIN" '$1==key {print $NF}')
GIT_PLUGIN="git"
GIT_PLUGIN_VERSION=$(java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN list-plugins | awk -v key="$GIT_PLUGIN" '$1==key {print $NF}')
...
discovery-news.xml | java -jar jenkins-cli.jar -s $JENKINS_EXTERNAL_URL -auth $JENKINS_USER_ID:$JENKINS_USER_API_TOKEN \
