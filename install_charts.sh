...
[ "$(helm list $JENKINS_HELM_RELEASE)" == "" ] && helm install --name $JENKINS_HELM_RELEASE --namespace $GITLAB_NAMESPACE --set Master.Image=jenkins/jenkins --set Master.ImageTag=lts --set Master.InstallPlugins={kubernetes:1.3.3\,workflow-aggregator:2.5\,workflow-job:2.17\,credentials-binding:1.15\,git:3.8.0\,gitlab-plugin:1.5.3} --set Agent.Image=jenkins/jnlp-slave --set Agent.ImageTag=latest --set rbac.install=true stable/jenkins
