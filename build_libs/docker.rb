...
module DockerTools
  ...
  def self.push_to_registry image_name, tag
    ...
    Shell.run %Q([ "$(docker images #{private_registry}/#{namespace}/#{image_name}:#{tag} -q)" != "" ] && docker rmi #{private_registry}/#{namespace}/#{image_name}:#{tag} || echo "No existing image found")
    Shell.run cmds
    ...
    if (t > 0)
      (1..t).each do |i|
        Shell.run %Q([ "$(docker images #{private_registry}/#{namespace}/#{image_name}:#{i} -q)" != "" ] && docker rmi #{private_registry}/#{namespace}/#{image_name}:#{i} || echo "No existing image found")
      end
    end    
  end
end
